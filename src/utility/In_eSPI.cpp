...
void gpioMode(uint8_t gpio, uint8_t mode) {
    ...
    ESP_REG(DR_REG_IO_MUX_BASE + esp32_gpioMux[gpio].reg) =
        ((uint32_t)2 << FUN_DRV_S) | (FUN_IE) | ((uint32_t)2 << MCU_SEL_S);
    GPIO.pin[gpio].val = 0;
}
#endif
...
void TFT_eSPI::drawCircle(int32_t x0, int32_t y0, int32_t r, uint32_t color) {
    ...
    // spi_begin();          // Sprite class can use this function, avoiding
    // spi_begin()
    inTransaction = true;
...
uint16_t TFT_eSPI::decodeUTF8(uint8_t c) {
    ...
    if (decoderState == 0) {
        ...
        // ASCII if ((c & 0xF8) == 0xF0) return (uint16_t)c;
    } else {
        if (decoderState == 2) {
...
//  //  //delay(5);                     // Debug optional wait for serial port
//  to flush through
//^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ DEBUG
//^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
int16_t TFT_eSPI::drawString(const char *string, int32_t poX, int32_t poY,
    ...
    if (freeFont && (textcolor != textbgcolor)) {
        ...
        if (is_in_block_flag == true)
        // if((c2 >= pgm_read_word(&gfxFont->first)) && (c2 <=
        // pgm_read_word(&gfxFont->last) ))
        {
            // c2 -= pgm_read_word(&gfxFont->first);
            GFXglyph *glyph =
...
void TFT_eSPI::setFreeFont(const GFXfont *f) {
    ...
    if (pgm_read_word(&gfxFont->range_num) != 0) {
        ...
        for (uint16_t i = 0; i < pgm_read_word(&gfxFont->range_num); i++) {
            numChars += pgm_read_word(&range_pst[i].end) -
                        pgm_read_word(&range_pst[i].start);
        }
    } else {
        numChars =
...
// This is part of the TFT_eSPI class and is associated with anti-aliased font
// functions
...
void TFT_eSPI::loadFont(String fontName, bool flash) {
    /*
      ...
      Next are gCount sets of values for each glyph, each set comprises 7 int32t
      parameters (28 bytes):
      ...
      value +ve = up)
        6. dX = distance from cursor to left side of glyph bitmap (signed value
      -ve = left)
      ...
      transparency from 0xFF foreground colour, 0x00 background. The sketch uses
      a linear interpolation between the foreground and background RGB component
      colours. e.g. pixelRed = ((fgRed * alpha) + (bgRed * (255 - alpha))/255 To
      gain a performance advantage fixed point arithmetic is used with rounding
      and division by 256 (shift right 8 bits is faster).
      ...
      // Cursor coordinate positions for this and next character are marked by
      'C'
      // C<------- gxAdvance ------->C  gxAdvance is how far to move cursor for
      next glyph cursor position
      // |                           |
      // |                           |   ascent is top of "d", descent is bottom
      of "p"
      // +-- gdX --+             ascent
      // |         +-- gWidth--+     |   gdX is offset to left edge of glyph
      bitmap
      // |   +     x@.........@x  +  |   gdX may be negative e.g. italic "y"
      tail extending to left of
      // |   |     @@.........@@  |  |   cursor position, plot top left corner
      of bitmap at (cursorX + gdX)
      // |   |     @@.........@@ gdY |   gWidth and gHeight are glyph bitmap
      dimensions
      // |   |     .@@@.....@@@@  |  |
      // | gHeight ....@@@@@..@@  +  +    <-- baseline
      // |   |     ...........@@     |
      // |   |     ...........@@     |   gdY is the offset to the top edge of
      the bitmap
      // |   |     .@@.......@@. descent plot top edge of bitmap at (cursorY +
      yAdvance - gdY)
      // |   +     x..@@@@@@@..x     |   x marks the corner pixels of the bitmap
      // |                           |
      // +---------------------------+   yAdvance is y delta for the next line,
      font size or (ascent + descent)
      //                                  some fonts can overlay in y direction
      so may need a user adjust value
...
void TFT_eSPI::loadMetrics(uint16_t gCount) {
    ...
    if (psramFound()) {
        gUnicode = (uint16_t *)ps_malloc(
            gCount * 2);  // Unicode 16 bit Basic Multilingual Plane (0-FFFF)
        gHeight = (uint8_t *)ps_malloc(gCount);  // Height of glyph
        gWidth  = (uint8_t *)ps_malloc(gCount);  // Width of glyph
        gxAdvance =
            (uint8_t *)ps_malloc(gCount);  // xAdvance - to move x cursor
        gdY = (int16_t *)ps_malloc(gCount *
                                   2);  // offset from bitmap top edge from
                                        // lowest point in any character
        gdX = (int8_t *)ps_malloc(
            gCount);  // offset for bitmap left edge relative to cursor X
        gBitmap = (uint32_t *)ps_malloc(
            gCount * 4);  // seek pointer to glyph bitmap in the file
    } else
#endif
    {
        gUnicode = (uint16_t *)malloc(
            gCount * 2);  // Unicode 16 bit Basic Multilingual Plane (0-FFFF)
        ...
        gdY =
            (int16_t *)malloc(gCount * 2);  // offset from bitmap top edge from
                                            // lowest point in any character
        gdX = (int8_t *)malloc(
            gCount);  // offset for bitmap left edge relative to cursor X
        gBitmap = (uint32_t *)malloc(
            gCount * 4);  // seek pointer to glyph bitmap in the file
    }
    ...
    while (gNum < gCount) {
        ...
        // Serial.print("Unicode = 0x"); Serial.print(gUnicode[gNum], HEX);
        // Serial.print(", gHeight  = "); Serial.println(gHeight[gNum]);
        // Serial.print("Unicode = 0x"); Serial.print(gUnicode[gNum], HEX);
        // Serial.print(", gWidth  = "); Serial.println(gWidth[gNum]);
        // Serial.print("Unicode = 0x"); Serial.print(gUnicode[gNum], HEX);
        // Serial.print(", gxAdvance  = "); Serial.println(gxAdvance[gNum]);
        // Serial.print("Unicode = 0x"); Serial.print(gUnicode[gNum], HEX);
        // Serial.print(", gdY  = "); Serial.println(gdY[gNum]);

        // Different glyph sets have different ascent values not always based on
        // "d", so we could get the maximum glyph ascent by checking all
        // characters. BUT this method can generate bad values for non-existant
        // glyphs, so we will reply on processing for the value and disable this
        // code for now...
        /*
        if (gdY[gNum] > gFont.maxAscent)
        {
          // Try to avoid UTF coding values and characters that tend to give
    duff values if (((gUnicode[gNum] > 0x20) && (gUnicode[gNum] < 0x7F)) ||
    (gUnicode[gNum] > 0xA0))
          {
            gFont.maxAscent   = gdY[gNum];
    #ifdef SHOW_ASCENT_DESCENT
            Serial.print("Unicode = 0x"); Serial.print(gUnicode[gNum], HEX);
    Serial.print(", maxAscent  = "); Serial.println(gFont.maxAscent); #endif
          }
        }
        */

        // Different glyph sets have different descent values not always based
        // on "p", so get maximum glyph descent
        if (((int16_t)gHeight[gNum] - (int16_t)gdY[gNum]) > gFont.maxDescent) {
            // Avoid UTF coding values and characters that tend to give duff
            // values
    ...
    gFont.spaceWidth =
        (gFont.ascent + gFont.descent) * 2 / 7;  // Guess at space width
}
...
** Description:             Blend foreground and background and return new
*colour
*************************************************************************************x*/
...
void TFT_eSPI::drawGlyph(uint16_t code) {
    ...
    if (found) {
        ...
        fontFile.seek(gBitmap[gNum],
                      fs::SeekSet);  // This is taking >30ms for a significant
                                     // position shift
        ...
        drawRect(cursor_x, cursor_y + gFont.maxAscent - gFont.ascent,
...
** Description:             Page through all characters in font, td ms between
*screens
*************************************************************************************x*/
...
int32_t TFT_eSPI::getUnicodeFontIndex(uint32_t unicode) {
    ...
    if (custom_range_num != 0) {
        if ((unicode >= pgm_read_word(&range_pst[0].start)) &&
            (unicode <= pgm_read_word(&range_pst[0].end))) {
            index = unicode - pgm_read_word(&range_pst[0].start) +
                    pgm_read_word(&range_pst[0].base);
        } else {
            ...
            while (left <= right) {
                ...
                if (unicode > pgm_read_word(&range_pst[middle].end)) {
                    ...
                    right = middle - 1;
                            ...
                            pgm_read_word(&range_pst[middle].start)) &&
                           (unicode <= pgm_read_word(&range_pst[middle].end))) {
                    index = unicode - pgm_read_word(&range_pst[middle].start) +
                            pgm_read_word(&range_pst[middle].base);
                    break;
                }
            }
        }
