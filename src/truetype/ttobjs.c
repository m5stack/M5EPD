...
#define TRICK_NAMES_COUNT          8
    ...
    for (nn = 0; nn < TRICK_NAMES_COUNT; nn++)
        if (ft_strstr(name, trick_names[nn])) return TRUE;
...
static FT_UInt32 tt_synth_sfnt_checksum(FT_Stream stream, FT_ULong length) {
    ...
    if (FT_FRAME_ENTER(length)) return 0;
    ...
    for (i = 0; i < face->num_tables; i++) {
        ...
        for (j = 0; j < TRICK_SFNT_IDS_NUM_FACES; j++)
            if (face->dir_tables[i].Length == sfnt_id[j][k].Length) {
                if (!checksum) checksum = tt_get_sfnt_checksum(face, i);

                if (sfnt_id[j][k].CheckSum == checksum) num_matched_ids[j]++;

                if (num_matched_ids[j] == TRICK_SFNT_IDS_PER_FACE) return TRUE;
            }
    }
...
static FT_Bool tt_check_trickyness(FT_Face face) {
    ...
    if (tt_check_trickyness_sfnt_ids((TT_Face)face)) return TRUE;
...
tt_face_init(FT_Stream stream, FT_Face ttface, /* TT_Face */
    ...
    if (tt_check_trickyness(ttface)) ttface->face_flags |= FT_FACE_FLAG_TRICKY;
    ...
    if (FT_IS_SCALABLE(ttface)) {
        ...
        if (!error) error = tt_face_load_cvt(face, stream);
        if (!error) error = tt_face_load_fpgm(face, stream);
        if (!error) error = tt_face_load_prep(face, stream);

        /* Check the scalable flag based on `loca'. */
        if (!ttface->internal->incremental_interface &&
            ttface->num_fixed_sizes && face->glyph_locations &&
        ...
        if (!error) error = tt_face_load_loca(face, stream);
        if (!error) error = tt_face_load_cvt(face, stream);
        if (!error) error = tt_face_load_fpgm(face, stream);
        if (!error) error = tt_face_load_prep(face, stream);
...
tt_face_done(FT_Face ttface) /* TT_Face */
{
    ...
    if (face->extra.finalizer) face->extra.finalizer(face->extra.data);

    if (sfnt) sfnt->done_face(face);
...
tt_size_run_fpgm(TT_Size size, FT_Bool pedantic) {
    ...
    if (!error) TT_Save_Context(exec, size);
...
static FT_Error tt_size_init_bytecode(FT_Size ftsize, FT_Bool pedantic) {
    ...
    /* Set default metrics */
    {
        ...
        for (i = 0; i < 4; i++) metrics->compensations[i] = 0;
    }
    ...
    /* set `face->interpreter' according to the debug hook present */
    {
        ...
        if (!face->interpreter) face->interpreter = (TT_Interpreter)TT_RunIns;
    }
...
Exit:
    if (error) tt_size_done_bytecode(ftsize);
...
tt_size_ready_bytecode(TT_Size size, FT_Bool pedantic) {
    ...
    if (!size->cvt_ready) {
        ...
        /* clear storage area */
        for (i = 0; i < (FT_UInt)size->storage_size; i++) size->storage[i] = 0;

        size->GS = tt_default_graphics_state;

        error = tt_size_run_prep(size, pedantic);
        if (!error) size->cvt_ready = 1;
    }
...
#ifdef TT_USE_BYTECODE_INTERPRETER
    if (size->bytecode_ready) tt_size_done_bytecode(ttsize);
#endif
...
tt_size_reset(TT_Size size) {
    ...
    *metrics = size->root.metrics;
    ...
    if (face->header.Flags & 8) {
        metrics->x_scale =
            FT_DivFix(metrics->x_ppem << 6, face->root.units_per_EM);
        metrics->y_scale =
            FT_DivFix(metrics->y_ppem << 6, face->root.units_per_EM);
        ...
        metrics->max_advance = FT_PIX_ROUND(
            FT_MulFix(face->root.max_advance_width, metrics->x_scale));
    }
    ...
    if (!error) size->ttmetrics.valid = TRUE;
...
tt_driver_init(FT_Module ttdriver) /* TT_Driver */
{
    ...
    if (!TT_New_Context(driver)) return FT_THROW(Could_Not_Find_Context);
