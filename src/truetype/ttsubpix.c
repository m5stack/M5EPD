...
const SPH_Font_Class STYLE_CLASS_Rules[STYLE_CLASS_RULES_SIZE] = {
    {
        "Regular Class",
        {
            "Regular",
...
const SPH_TweakRule COMPATIBILITY_MODE_Rules[COMPATIBILITY_MODE_RULES_SIZE] = {
    {"-", 0, "", 0},
};
...
const SPH_TweakRule PIXEL_HINTING_Rules[PIXEL_HINTING_RULES_SIZE] = {
...
const SPH_TweakRule SKIP_INLINE_DELTAS_Rules[SKIP_INLINE_DELTAS_RULES_SIZE] = {
    {"-", 0, "", 0},
};
...
const SPH_TweakRule DO_SHPIX_Rules[DO_SHPIX_RULES_SIZE] = {
    {"-", 0, "", 0},
};
...
const SPH_TweakRule
    SKIP_NONPIXEL_Y_MOVES_Rules[SKIP_NONPIXEL_Y_MOVES_RULES_SIZE] = {
        /* fix vwxyz thinness*/
...
const SPH_TweakRule
    SKIP_OFFPIXEL_Y_MOVES_Rules[SKIP_OFFPIXEL_Y_MOVES_RULES_SIZE] = {
        {"-", 0, "", 0},
};
...
const SPH_TweakRule
    ROUND_NONPIXEL_Y_MOVES_Rules[ROUND_NONPIXEL_Y_MOVES_RULES_SIZE] = {
...
const SPH_TweakRule ALLOW_X_DMOVEX_Rules[ALLOW_X_DMOVEX_RULES_SIZE] = {
    {"-", 0, "Regular", 0},
};
...
const SPH_TweakRule ALLOW_X_DMOVE_Rules[ALLOW_X_DMOVE_RULES_SIZE] = {
...
const SPH_TweakRule ALLOW_X_MOVE_ZP2_Rules[ALLOW_X_MOVE_ZP2_RULES_SIZE] = {
    {"-", 0, "", 0},
};
...
const SPH_TweakRule RASTERIZER_35_Rules[RASTERIZER_35_RULES_SIZE] = {
...
const SPH_TweakRule NORMAL_ROUND_Rules[NORMAL_ROUND_RULES_SIZE] = {
    /* Fix serif thickness */
    {"Courier New", 0, "", 0},
};
...
const SPH_TweakRule SKIP_IUP_Rules[SKIP_IUP_RULES_SIZE] = {
    {"Arial", 13, "Regular", 'a'},
};
...
const SPH_TweakRule MIAP_HACK_Rules[MIAP_HACK_RULES_SIZE] = {
    {"Geneva", 12, "", 0},
};
...
const SPH_TweakRule ALWAYS_SKIP_DELTAP_Rules[ALWAYS_SKIP_DELTAP_RULES_SIZE] = {
    {"Georgia", 0, "Regular", 'k'},
...
const SPH_TweakRule ALWAYS_DO_DELTAP_Rules[ALWAYS_DO_DELTAP_RULES_SIZE] = {
...
static const SPH_TweakRule DELTAP_RTG_Rules[DELTAP_RTG_RULES_SIZE] = {
    {"-", 0, "", 0},
};
...
static const SPH_TweakRule MIRP_CVT_ZERO_Rules[MIRP_CVT_ZERO_RULES_SIZE] = {
    {"-", 0, "", 0},
};
...
static const SPH_TweakRule
    NO_ALIGNRP_AFTER_IUP_Rules[NO_ALIGNRP_AFTER_IUP_RULES_SIZE] = {
...
static const SPH_TweakRule
    NO_DELTAP_AFTER_IUP_Rules[NO_DELTAP_AFTER_IUP_RULES_SIZE] = {
        {"-", 0, "", 0},
};
...
static const SPH_TweakRule
    NO_CALL_AFTER_IUP_Rules[NO_CALL_AFTER_IUP_RULES_SIZE] = {
        /* Prevent creation of dents in outline */
...
static const SPH_TweakRule DEEMBOLDEN_Rules[DEEMBOLDEN_RULES_SIZE] = {
    {"Courier New", 0, "Bold", 'A'}, {"Courier New", 0, "Bold", 'W'},
    {"Courier New", 0, "Bold", 'w'}, {"Courier New", 0, "Bold", 'M'},
    {"Courier New", 0, "Bold", 'X'}, {"Courier New", 0, "Bold", 'K'},
    {"Courier New", 0, "Bold", 'x'}, {"Courier New", 0, "Bold", 'z'},
...
static const SPH_TweakRule EMBOLDEN_Rules[EMBOLDEN_RULES_SIZE] = {
    {"Courier New", 12, "Italic", 'z'}, {"Courier New", 11, "Italic", 'z'},
    {"Courier New", 10, "Italic", 'z'}, {"Courier New", 0, "Regular", 0},
...
static const SPH_TweakRule DELTAP_RDTG_Rules[DELTAP_RDTG_RULES_SIZE] = {
    {"-", 0, "", 0},
};
...
static const SPH_TweakRule
    TIMES_NEW_ROMAN_HACK_Rules[TIMES_NEW_ROMAN_HACK_RULES_SIZE] = {
...
static const SPH_TweakRule
    COURIER_NEW_2_HACK_Rules[COURIER_NEW_2_HACK_RULES_SIZE] = {
    ...
    for (i = 0; i < FAMILY_CLASS_RULES_SIZE; i++) {
        if (strcmp(FAMILY_CLASS_Rules[i].name, rule_font_name) == 0) {
            for (j = 0; j < SPH_MAX_CLASS_MEMBERS; j++) {
                if (strcmp(FAMILY_CLASS_Rules[i].member[j], "") == 0) continue;
...
is_member_of_style_class(const FT_String* detected_font_style,
    ...
    for (i = 0; i < STYLE_CLASS_RULES_SIZE; i++) {
        if (strcmp(STYLE_CLASS_Rules[i].name, rule_font_style) == 0) {
            for (j = 0; j < SPH_MAX_CLASS_MEMBERS; j++) {
                if (strcmp(STYLE_CLASS_Rules[i].member[j], "") == 0) continue;
...
FT_LOCAL_DEF(FT_Bool)
sph_test_tweak(TT_Face face, const FT_String* family, FT_UInt ppem,
               const FT_String* style, FT_UInt glyph_index,
...
static FT_UInt scale_test_tweak(TT_Face face, const FT_String* family,
                                FT_UInt ppem, const FT_String* style,
                                FT_UInt glyph_index, const SPH_ScaleRule* rule,
                                FT_UInt num_rules) {
    FT_UInt i;
...
sph_test_tweak_x_scaling(TT_Face face, const FT_String* family, FT_UInt ppem,
                         const FT_String* style, FT_UInt glyph_index) {
...
#define TWEAK_RULES(x)                                                    \
    if (sph_test_tweak(face, family, ppem, style, glyph_index, x##_Rules, \
                       x##_RULES_SIZE))                                   \
...
sph_set_tweaks(TT_Loader loader, FT_UInt glyph_index) {
    ...
    if (loader->exec->sph_tweak_flags & SPH_TWEAK_RASTERIZER_35) {
        ...
        if (loader->exec->rasterizer_version !=
            ...
            loader->exec->rasterizer_version =
                SPH_OPTION_SET_RASTERIZER_VERSION;
            loader->exec->size->cvt_ready = FALSE;
