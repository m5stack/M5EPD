...
tt_face_load_loca(TT_Face face, FT_Stream stream) {
    ...
    if (face->num_locations != (FT_ULong)face->root.num_glyphs + 1) {
        ...
        if (face->num_locations <= (FT_ULong)face->root.num_glyphs) {
            FT_Long new_loca_len = ((FT_Long)(face->root.num_glyphs) + 1)
                                   << shift;
     ...
     */
    if (FT_FRAME_EXTRACT(table_len, face->glyph_locations)) goto Exit;
...
tt_face_get_location(TT_Face face, FT_UInt gindex, FT_UInt* asize) {
    ...
    if (gindex < face->num_locations) {
        if (face->header.Index_To_Loc_Format != 0) {
            ...
            if (p + 4 <= p_limit) pos2 = FT_NEXT_ULONG(p);
        } else {
            ...
            if (p + 2 <= p_limit) pos2 = FT_NEXT_USHORT(p);
...
tt_face_load_cvt(TT_Face face, FT_Stream stream) {
    ...
    if (FT_NEW_ARRAY(face->cvt, face->cvt_size)) goto Exit;

    if (FT_FRAME_ENTER(face->cvt_size * 2L)) goto Exit;

    {
        FT_Short* cur   = face->cvt;
        FT_Short* limit = cur + face->cvt_size;

        for (; cur < limit; cur++) *cur = FT_GET_SHORT();
    }
...
#ifdef TT_CONFIG_OPTION_GX_VAR_SUPPORT
    if (face->doblend) error = tt_face_vary_cvt(face, stream);
#endif
...
tt_face_load_fpgm(TT_Face face, FT_Stream stream) {
    ...
    if (error) {
        ...
        if (FT_FRAME_EXTRACT(table_len, face->font_program)) goto Exit;
...
tt_face_load_prep(TT_Face face, FT_Stream stream) {
    ...
    if (error) {
        ...
        if (FT_FRAME_EXTRACT(table_len, face->cvt_program)) goto Exit;
...
tt_face_load_hdmx(TT_Face face, FT_Stream stream) {
    ...
    if (error || table_size < 8) return FT_Err_Ok;

    if (FT_FRAME_EXTRACT(table_size, face->hdmx_table)) goto Exit;
    ...
    if (record_size >= 0xFFFF0000UL) record_size &= 0xFFFFU;
    ...
    if (FT_NEW_ARRAY(face->hdmx_record_sizes, num_records)) goto Fail;

    for (nn = 0; nn < num_records; nn++) {
        if (p + record_size > limit) break;
