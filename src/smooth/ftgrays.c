...
#define FT_THROW(e)                                           \
    (FT_Throw(FT_ERR_CAT(ErrRaster, e), __LINE__, __FILE__) | \
...
static void gray_render_line(RAS_ARG_ TPos to_x, TPos to_y) {
    ...
    /* perform vertical clipping */
    {
        ...
        if (min >= ras.max_ey || max < ras.min_ey) goto End;
    }
    ...
    if (ey1 != ey2) {
        ...
        while (ey1 != ey2) {
            ...
            x2 = x + delta;
            gray_render_scanline(RAS_VAR_ ey1, x, (TCoord)(ONE_PIXEL - first),
                                 x2, (TCoord)first);
            x = x2;
    ...
    gray_render_scanline(RAS_VAR_ ey1, x, (TCoord)(ONE_PIXEL - first), to_x,
                         fy2);
...
static void gray_render_span(int y, int count, const FT_Span* spans,
                             gray_PWorker worker) {
    unsigned char* p;
    ...
    if (map->pitch >= 0) p += (unsigned)((map->rows - 1) * map->pitch);
...
static void gray_hline(RAS_ARG_ TCoord x, TCoord y, TPos area, TCoord acount) {
    ...
    if (y >= FT_INT_MAX) y = FT_INT_MAX;

    if (coverage) {
        /* see whether we can add this span to the current list */
        count = ras.num_gray_spans;
        span  = ras.gray_spans + count - 1;
        if (count > 0 && ras.span_y == y &&
            (int)span->x + span->len == (int)x && span->coverage == coverage) {
...
static void gray_sweep(RAS_ARG_ const FT_Bitmap* target) {
    ...
    if (ras.render_span && ras.num_gray_spans > 0)
        ras.render_span(ras.span_y, ras.num_gray_spans, ras.gray_spans,
                        ras.render_span_data);
    ...
    if (ras.num_gray_spans > 0) {
        ...
        for (n = 0; n < ras.num_gray_spans; n++, span++)
            FT_TRACE7(("[%d..%d]:%02x ", span->x, span->x + span->len - 1,
                       span->coverage));
        FT_TRACE7(("\n"));
    }
...
#define SCALED(x) (((x) << shift) - delta)
    ...
    for (n = 0; n < outline->n_contours; n++) {
        ...
        if (tag == FT_CURVE_TAG_CUBIC) goto Invalid_Outline;
            ...
            switch (tag) {
                ...
                Do_Conic:
                    if (point < limit) {
                        ...
                        if (tag == FT_CURVE_TAG_ON) {
                            FT_TRACE5(
                                ("  conic to (%.2f, %.2f)"
                                 " with control (%.2f, %.2f)\n",
                                 vec.x / 64.0, vec.y / 64.0, v_control.x / 64.0,
                                 v_control.y / 64.0));
                        ...
                        if (tag != FT_CURVE_TAG_CONIC) goto Invalid_Outline;
                    ...
                    FT_TRACE5(
                        ("  conic to (%.2f, %.2f)"
                         " with control (%.2f, %.2f)\n",
                         v_start.x / 64.0, v_start.y / 64.0, v_control.x / 64.0,
                         v_control.y / 64.0));
                    error =
                ...
                default: /* FT_CURVE_TAG_CUBIC */
                {
                    ...
                    FT_TRACE5(
                        ("  cubic to (%.2f, %.2f)"
                         " with controls (%.2f, %.2f) and (%.2f, %.2f)\n",
                         v_start.x / 64.0, v_start.y / 64.0, vec1.x / 64.0,
                         vec1.y / 64.0, vec2.x / 64.0, vec2.y / 64.0));
                    error =
...
FT_DEFINE_OUTLINE_FUNCS(func_interface, (FT_Outline_MoveTo_Func)gray_move_to,
                        (FT_Outline_LineTo_Func)gray_line_to,
                        (FT_Outline_ConicTo_Func)gray_conic_to,
                        (FT_Outline_CubicTo_Func)gray_cubic_to, 0, 0)
...
static int gray_raster_render(gray_PRaster raster,
    ...
    if (!(params->flags & FT_RASTER_FLAG_DIRECT)) {
        if (!target_map) return FT_THROW(Invalid_Argument);

        /* nothing to do */
        if (!target_map->width || !target_map->rows) return 0;

        if (!target_map->buffer) return FT_THROW(Invalid_Argument);
    }

    /* this version does not support monochrome rendering */
    if (!(params->flags & FT_RASTER_FLAG_AA)) return FT_THROW(Invalid_Mode);
...
static void gray_raster_reset(FT_Raster raster, char* pool_base,
                              long pool_size) {
    ...
    if (raster) {
        if (pool_base && pool_size >= (long)sizeof(gray_TWorker) + 2048) {
            ...
            rast->worker = worker;
            rast->buffer =
                pool_base + ((sizeof(gray_TWorker) + sizeof(TCell) - 1) &
                             ~(sizeof(TCell) - 1));
            rast->buffer_size =
                (long)((pool_base + pool_size) - (char*)rast->buffer) &
                ~(sizeof(TCell) - 1);
...
FT_DEFINE_RASTER_FUNCS(ft_grays_raster, FT_GLYPH_FORMAT_OUTLINE,
