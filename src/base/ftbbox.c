...
static void test_cubic_extrema(FT_Pos y1, FT_Pos y2, FT_Pos y3, FT_Pos y4,
                               FT_Fixed u, FT_Pos* min, FT_Pos* max) {
...
static void BBox_Cubic_Check(FT_Pos y1, FT_Pos y2, FT_Pos y3, FT_Pos y4,
    ...
    if (y1 <= y4) {
        ...
        if (y1 <= y2 && y2 <= y4 && y1 <= y3 && y3 <= y4) return;
    } else /* y1 > y4 */
    {
        /* descending arc test */
        if (y1 >= y2 && y2 >= y4 && y1 >= y3 && y3 >= y4) return;
...
FT_DEFINE_OUTLINE_FUNCS(bbox_interface, (FT_Outline_MoveTo_Func)BBox_Move_To,
