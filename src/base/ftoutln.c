...
FT_Outline_New_Internal(FT_Memory memory, FT_UInt numPoints, FT_Int numContours,
    ...
    if (!anoutline || !memory) return FT_THROW(Invalid_Argument);
    ...
    *anoutline = null_outline;
    ...
    if (numPoints > FT_OUTLINE_POINTS_MAX) return FT_THROW(Array_Too_Large);
...
FT_Outline_New(FT_Library library, FT_UInt numPoints, FT_Int numContours,
    ...
    return FT_Outline_New_Internal(library->memory, numPoints, numContours,
                                   anoutline);
}
...
FT_Outline_Copy(const FT_Outline* source, FT_Outline* target) {
    ...
    if (source == target) return FT_Err_Ok;
...
FT_EXPORT_DEF(void)
FT_Outline_Translate(const FT_Outline* outline, FT_Pos xOffset,
                     FT_Pos yOffset) {
    FT_UShort n;
    FT_Vector* vec;
...
FT_Outline_Render(FT_Library library, FT_Outline* outline,
    ...
    if (!outline || !params) return FT_THROW(Invalid_Argument);
    ...
    if (!error && update && renderer) FT_Set_Renderer(library, renderer, 0, 0);
...
FT_Outline_Get_Bitmap(FT_Library library, FT_Outline* outline,
    ...
    if (!abitmap) return FT_THROW(Invalid_Argument);
...
FT_Outline_Transform(const FT_Outline* outline, const FT_Matrix* matrix) {
    ...
    for (; vec < limit; vec++) FT_Vector_Transform(vec, matrix);
}
...
FT_Outline_EmboldenXY(FT_Outline* outline, FT_Pos xstrength, FT_Pos ystrength) {
    ...
    xstrength /= 2;
    ystrength /= 2;
    if (xstrength == 0 && ystrength == 0) return FT_Err_Ok;
    ...
    for (c = 0; c < outline->n_contours; c++) {
        ...
        for (n = first; n <= last; n++) {
            ...
            if (d > -0xF000L) {
                ...
                if (orientation == FT_ORIENTATION_TRUETYPE) q = -q;
...
FT_Outline_Get_Orientation(FT_Outline* outline) {
    ...
    if (!outline || outline->n_points <= 0) return FT_ORIENTATION_TRUETYPE;
