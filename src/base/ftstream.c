...
FT_Stream_Close(FT_Stream stream) {
    if (stream && stream->close) stream->close(stream);
}
...
FT_Stream_Skip(FT_Stream stream, FT_Long distance) {
    if (distance < 0) return FT_THROW(Invalid_Stream_Operation);
...
FT_Stream_ReadAt(FT_Stream stream, FT_ULong pos, FT_Byte* buffer,
    ...
    if (stream->read)
        ...
        if (read_bytes > count) read_bytes = count;
...
FT_Stream_TryRead(FT_Stream stream, FT_Byte* buffer, FT_ULong count) {
    ...
    if (stream->pos >= stream->size) goto Exit;
...
FT_Stream_EnterFrame(FT_Stream stream, FT_ULong count) {
    ...
    if (stream->read) {
        ...
        if (error) goto Exit;
#else
        if (FT_QALLOC(stream->base, count)) goto Exit;
#endif
        /* read it */
...
FT_Stream_GetChar(FT_Stream stream) {
    ...
    result = 0;
    if (stream->cursor < stream->limit) result = *stream->cursor++;
...
FT_Stream_GetUShort(FT_Stream stream) {
    ...
    result = 0;
    p      = stream->cursor;
    if (p + 1 < stream->limit) result = FT_NEXT_USHORT(p);
    stream->cursor = p;
...
FT_Stream_GetUShortLE(FT_Stream stream) {
    ...
    result = 0;
    p      = stream->cursor;
    if (p + 1 < stream->limit) result = FT_NEXT_USHORT_LE(p);
    stream->cursor = p;
...
FT_Stream_GetUOffset(FT_Stream stream) {
    ...
    result = 0;
    p      = stream->cursor;
    if (p + 2 < stream->limit) result = FT_NEXT_UOFF3(p);
    stream->cursor = p;
    return result;
}
...
FT_Stream_GetULong(FT_Stream stream) {
    ...
    result = 0;
    p      = stream->cursor;
    if (p + 3 < stream->limit) result = FT_NEXT_ULONG(p);
    stream->cursor = p;
    return result;
}
...
FT_Stream_GetULongLE(FT_Stream stream) {
    ...
    result = 0;
    p      = stream->cursor;
    if (p + 3 < stream->limit) result = FT_NEXT_ULONG_LE(p);
    stream->cursor = p;
    return result;
}
...
FT_Stream_ReadChar(FT_Stream stream, FT_Error* error) {
    ...
    *error = FT_Err_Ok;
    ...
    if (stream->read) {
        if (stream->read(stream, stream->pos, &result, 1L) != 1L) goto Fail;
    } else {
...
FT_Stream_ReadUShort(FT_Stream stream, FT_Error* error) {
    ...
    *error = FT_Err_Ok;
    ...
    if (stream->pos + 1 < stream->size) {
        if (stream->read) {
            if (stream->read(stream, stream->pos, reads, 2L) != 2L) goto Fail;
...
FT_Stream_ReadUShortLE(FT_Stream stream, FT_Error* error) {
    ...
    *error = FT_Err_Ok;
    ...
    if (stream->pos + 1 < stream->size) {
        ...
        if (p) result = FT_NEXT_USHORT_LE(p);
    } else
        goto Fail;
...
FT_Stream_ReadUOffset(FT_Stream stream, FT_Error* error) {
    ...
    *error = FT_Err_Ok;
    ...
    if (stream->pos + 2 < stream->size) {
        if (stream->read) {
            if (stream->read(stream, stream->pos, reads, 3L) != 3L) goto Fail;
...
FT_Stream_ReadULong(FT_Stream stream, FT_Error* error) {
    ...
    *error = FT_Err_Ok;
    ...
    if (stream->pos + 3 < stream->size) {
        if (stream->read) {
            if (stream->read(stream, stream->pos, reads, 4L) != 4L) goto Fail;
...
FT_Stream_ReadULongLE(FT_Stream stream, FT_Error* error) {
    ...
    *error = FT_Err_Ok;
    ...
    if (stream->pos + 3 < stream->size) {
        ...
        if (p) result = FT_NEXT_ULONG_LE(p);
    } else
        goto Fail;
...
FT_Stream_ReadFields(FT_Stream stream, const FT_Frame_Field* fields,
    ...
    if (!fields || !stream) return FT_THROW(Invalid_Argument);
...
Exit:
    ...
    if (frame_accessed) FT_Stream_ExitFrame(stream);
