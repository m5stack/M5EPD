...
ft_mem_alloc(FT_Memory memory, FT_Long size, FT_Error* p_error) {
    ...
    if (!error && size > 0) FT_MEM_ZERO(block, size);
...
ft_mem_qalloc(FT_Memory memory, FT_Long size, FT_Error* p_error) {
    ...
    if (size > 0) {
        ...
        if (block == NULL) error = FT_THROW(Out_Of_Memory);
    } else if (size < 0) {
...
ft_mem_free(FT_Memory memory, const void* P) {
    if (P) memory->free(memory, (void*)P);
}
...
ft_mem_dup(FT_Memory memory, const void* address, FT_ULong size,
    ...
    if (!error && address) ft_memcpy(p, address, size);
...
FT_List_Finalize(FT_List list, FT_List_Destructor destroy, FT_Memory memory,
    ...
    while (cur) {
        ...
        if (destroy) destroy(memory, data, user);
