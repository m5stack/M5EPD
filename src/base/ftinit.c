...
#define FT_USE_MODULE(type, x)                                               \
    FT_EXTERNC FT_Error FT_Create_Class_##x(FT_Library library,              \
                                            FT_Module_Class** output_class); \
    FT_EXTERNC void FT_Destroy_Class_##x(FT_Library library,                 \
...
ft_destroy_default_module_classes(FT_Library library) {
    ...
    if (!pic_container->default_module_classes) return;
...
ft_create_default_module_classes(FT_Library library) {
    ...
    if (FT_ALLOC(classes,
                 sizeof(FT_Module_Class*) * (FT_NUM_MODULE_CLASSES + 1)))
        return error;

    /* initialize all pointers to 0, especially the last one */
    for (i = 0; i < FT_NUM_MODULE_CLASSES; i++) classes[i] = 0;
