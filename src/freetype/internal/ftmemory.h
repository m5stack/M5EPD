...
#define FT_MEM_REALLOC(ptr, cursz, newsz) \
    FT_ASSIGNP_INNER(                     \
        ptr, ft_mem_realloc(memory, 1, (cursz), (newsz), (ptr), &error))
...
#define FT_MEM_QREALLOC(ptr, cursz, newsz) \
    FT_ASSIGNP_INNER(                      \
        ptr, ft_mem_qrealloc(memory, 1, (cursz), (newsz), (ptr), &error))

#define FT_MEM_QRENEW_ARRAY(ptr, cursz, newsz)                             \
    FT_ASSIGNP_INNER(ptr, ft_mem_qrealloc(memory, sizeof(*(ptr)), (cursz), \
                                          (newsz), (ptr), &error))

#define FT_MEM_ALLOC_MULT(ptr, count, item_size) \
    FT_ASSIGNP_INNER(                            \
        ptr, ft_mem_realloc(memory, (item_size), 0, (count), NULL, &error))

#define FT_MEM_REALLOC_MULT(ptr, oldcnt, newcnt, itmsz)                       \
    FT_ASSIGNP_INNER(ptr, ft_mem_realloc(memory, (itmsz), (oldcnt), (newcnt), \
                                         (ptr), &error))

#define FT_MEM_QALLOC_MULT(ptr, count, item_size) \
    FT_ASSIGNP_INNER(                             \
        ptr, ft_mem_qrealloc(memory, (item_size), 0, (count), NULL, &error))

#define FT_MEM_QREALLOC_MULT(ptr, oldcnt, newcnt, itmsz)                       \
    FT_ASSIGNP_INNER(ptr, ft_mem_qrealloc(memory, (itmsz), (oldcnt), (newcnt), \
                                          (ptr), &error))
...
#define FT_MEM_NEW_ARRAY(ptr, count) \
    FT_ASSIGNP_INNER(                \
        ptr, ft_mem_realloc(memory, sizeof(*(ptr)), 0, (count), NULL, &error))

#define FT_MEM_RENEW_ARRAY(ptr, cursz, newsz)                             \
    FT_ASSIGNP_INNER(ptr, ft_mem_realloc(memory, sizeof(*(ptr)), (cursz), \
                                         (newsz), (ptr), &error))

#define FT_MEM_QNEW_ARRAY(ptr, count)                                         \
    FT_ASSIGNP_INNER(ptr, ft_mem_qrealloc(memory, sizeof(*(ptr)), 0, (count), \
                                          NULL, &error))
