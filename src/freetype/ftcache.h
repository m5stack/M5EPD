...
#define FTC_IMAGE_TYPE_HASH(d)                                      \
    (FT_UFast)(FTC_FACE_ID_HASH((d)->face_id) ^ ((d)->width << 8) ^ \
               (d)->height ^ ((d)->flags << 4))
...
FT_EXPORT(FT_Error)
FTC_Manager_Lookup_Face(FTC_Manager manager, FTC_FaceID face_id,
                        FT_Face *aface);

FT_EXPORT(FT_Error)
FTC_Manager_Lookup_Size(FTC_Manager manager, FTC_Font font, FT_Face *aface,
                        FT_Size *asize);
