...
tt_face_load_hmtx(TT_Face face, FT_Stream stream, FT_Bool vertical) {
    ...
    if (FT_FRAME_ENTER(table_len)) goto Fail;
    ...
    /* do we have an inconsistent number of metric values? */
    {
        ...
        for (; cur < limit; cur++) *cur = FT_NEXT_SHORT(p);
...
#define FT_STRUCTURE TT_HoriHeader
    ...
    if (FT_STREAM_READ_FIELDS(metrics_header_fields, header)) goto Fail;
...
tt_face_get_metrics(TT_Face face, FT_Bool vertical, FT_UInt gindex,
    ...
    if (k > 0) {
        if (gindex < (FT_UInt)k) {
            table_pos += 4 * gindex;
            if (table_pos + 4 > table_end) goto NoData;
            ...
            if (table_pos + 2 > table_end)
                *abearing = 0;
                ...
                if (!FT_STREAM_SEEK(table_pos)) (void)FT_READ_SHORT(*abearing);
            }
        }
    } else {
    NoData:
...
tt_face_get_metrics(TT_Face face, FT_Bool vertical, FT_UInt gindex,
    ...
    TT_HoriHeader* header = vertical ? (TT_HoriHeader*)v : (TT_HoriHeader*)h;
