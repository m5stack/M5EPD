...
tt_face_load_kern(TT_Face face, FT_Stream stream) {
    ...
    for (nn = 0; nn < num_tables; nn++) {
        ...
        if ((coverage & ~8) != 0x0001 || p + 8 > p_limit) goto NextTable;
        ...
        if (num_pairs > 0) {
            ...
            for (count = num_pairs - 1; count > 0; count--) {
                ...
                if (cur_pair <= old_pair) break;

                p += 2;
                old_pair = cur_pair;
            }

            if (count == 0) ordered |= mask;
        }
...
tt_face_get_kerning(TT_Face face, FT_UInt left_glyph, FT_UInt right_glyph) {
    ...
    for (count = face->num_kern_tables; count > 0 && p + 6 <= p_limit;
        ...
        if ((face->kern_avail_bits & mask) == 0) goto NextTable;

        if (p + 8 > next) goto NextTable;
