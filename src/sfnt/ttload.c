...
tt_face_goto_table(TT_Face face, FT_ULong tag, FT_Stream stream,
    ...
    if (table) {
        ...
        if (FT_STREAM_SEEK(table->Offset)) goto Exit;
    } else
...
#define FT_STRUCTURE SFNT_HeaderRec
    ...
    if (FT_QNEW_ARRAY(face->dir_tables, face->num_tables)) goto Exit;
...
tt_face_load_any(TT_Face face, FT_ULong tag, FT_Long offset, FT_Byte* buffer,
    ...
    if (FT_STREAM_READ_AT(offset, buffer, size)) goto Exit;
...
#define FT_STRUCTURE TT_Header
    ...
    if (FT_STREAM_READ_FIELDS(header_fields, header)) goto Exit;
...
#define FT_STRUCTURE TT_MaxProfile
    ...
    if (FT_STREAM_READ_FIELDS(maxp_fields, maxProfile)) goto Exit;
    ...
    if (maxProfile->version >= 0x10000L) {
        if (FT_STREAM_READ_FIELDS(maxp_fields_extra, maxProfile)) goto Exit;
        ...
        if (maxProfile->maxFunctionDefs < 64) maxProfile->maxFunctionDefs = 64;
...
#define FT_STRUCTURE TT_NameEntryRec
    ...
    if (FT_STREAM_READ_FIELDS(name_table_fields, table)) goto Exit;
    ...
    /* to hold the strings themselves.                                */
    {
        ...
        for (; count > 0; count--) {
            if (FT_STREAM_READ_FIELDS(name_record_fields, entry)) continue;

            /* check that the name is not empty */
            if (entry->stringLength == 0) continue;
...
#define FT_STRUCTURE TT_OS2
    ...
    if (FT_STREAM_READ_FIELDS(os2_fields, os2)) goto Exit;
    ...
    if (os2->version >= 0x0001) {
        ...
        if (FT_STREAM_READ_FIELDS(os2_fields_extra, os2)) goto Exit;

        if (os2->version >= 0x0002) {
            /* only version 2 tables */
            if (FT_STREAM_READ_FIELDS(os2_fields_extra2, os2)) goto Exit;
        }
    }
...
#define FT_STRUCTURE TT_Postscript
    ...
    if (FT_STREAM_READ_FIELDS(post_fields, post)) return error;
...
#define FT_STRUCTURE TT_PCLT
        ...
        FT_FRAME_START(54),           FT_FRAME_ULONG(Version),
        FT_FRAME_ULONG(FontNumber),   FT_FRAME_USHORT(Pitch),
        FT_FRAME_USHORT(xHeight),     FT_FRAME_USHORT(Style),
        FT_FRAME_USHORT(TypeFamily),  FT_FRAME_USHORT(CapHeight),
        FT_FRAME_BYTES(TypeFace, 16), FT_FRAME_BYTES(CharacterComplement, 8),
        FT_FRAME_BYTES(FileName, 6),  FT_FRAME_CHAR(StrokeWeight),
        FT_FRAME_CHAR(WidthType),     FT_FRAME_BYTE(SerifStyle),
    ...
    if (FT_STREAM_READ_FIELDS(pclt_fields, pclt)) goto Exit;
...
tt_face_load_gasp(TT_Face face, FT_Stream stream) {
    ...
    if (FT_FRAME_ENTER(4L)) goto Exit;
