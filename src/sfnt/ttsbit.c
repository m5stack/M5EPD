...
/*               direction!                                              */
/*                                                                       */
static void blit_sbit(FT_Bitmap* target, FT_Byte* source, FT_Int line_bits,
                      FT_Bool byte_padded, FT_Int x_offset, FT_Int y_offset,
    ...
    if (line_incr < 0) line_buff -= line_incr * (target->rows - 1);
    ...
    for (height = source_height; height > 0; height--) {
        ...
        if (count >= 8) {
            count -= 8;
            {
                do {
                    ...
                    if (loaded < 8) {
                        acc |=
                            (FT_UShort)((FT_UShort)*source++ << (8 - loaded));
                        loaded += 8;
                    }
        ...
        if (count > 0) {
            ...
            if (count > space) cur[1] |= (FT_Byte)(val << space);
...
#define FT_STRUCTURE TT_SBit_MetricsRec
    ...
    FT_FRAME_START(8),           FT_FRAME_BYTE(height),
    FT_FRAME_BYTE(width),

    FT_FRAME_CHAR(horiBearingX), FT_FRAME_CHAR(horiBearingY),
    FT_FRAME_BYTE(horiAdvance),

    FT_FRAME_CHAR(vertBearingX), FT_FRAME_CHAR(vertBearingY),
    FT_FRAME_BYTE(vertAdvance),  FT_FRAME_END};
...
static FT_Error Load_SBit_Const_Metrics(TT_SBit_Range range, FT_Stream stream) {
    ...
    if (FT_READ_ULONG(range->image_size)) return error;
...
/*    FreeType error code.  0 means success.                             */
/*                                                                       */
static FT_Error Load_SBit_Range_Codes(TT_SBit_Range range, FT_Stream stream,
                                      FT_Bool load_offsets) {
    FT_Error error;
    FT_ULong count, n, size;
    FT_Memory memory = stream->memory;

    if (FT_READ_ULONG(count)) goto Exit;

    range->num_glyphs = count;

    /* Allocate glyph offsets table if needed */
    if (load_offsets) {
        if (FT_NEW_ARRAY(range->glyph_offsets, count)) goto Exit;
    ...
    for (n = 0; n < count; n++) {
        ...
        if (load_offsets)
            range->glyph_offsets[n] =
                (FT_ULong)range->image_offset + FT_GET_USHORT();
    }
...
static FT_Error Load_SBit_Range(TT_SBit_Range range, FT_Stream stream) {
    ...
    switch (range->index_format) {
        ...
        case 5:
            error = Load_SBit_Const_Metrics(range, stream);
            if (!error) error = Load_SBit_Range_Codes(range, stream, 0);
            break;
...
#define FT_STRUCTURE TT_SBit_StrikeRec
        ...
        /* no FT_FRAME_START */
        FT_FRAME_ULONG(ranges_offset), FT_FRAME_SKIP_LONG,
        FT_FRAME_ULONG(num_ranges), FT_FRAME_ULONG(color_ref), FT_FRAME_END};
    ...
    if (error) error = face->goto_table(face, TTAG_bloc, stream, 0);
    if (error) goto Exit;

    table_base = FT_STREAM_POS();
    if (FT_FRAME_ENTER(8L)) goto Exit;
    ...
    if (FT_NEW_ARRAY(face->sbit_strikes, num_strikes)) goto Exit;
    ...
    /* now read each strike table separately */
    {
        ...
        if (FT_FRAME_ENTER(48L * num_strikes)) goto Exit;

        while (count > 0) {
            if (FT_STREAM_READ_FIELDS(strike_start_fields, strike) ||
                FT_STREAM_READ_FIELDS(sbit_line_metrics_fields,
                                      &strike->hori) ||
                FT_STREAM_READ_FIELDS(sbit_line_metrics_fields,
                                      &strike->vert) ||
    ...
    /* allocate the index ranges for each strike table */
    {
        ...
        while (count > 0) {
            ...
            while (count2 > 0) {
                ...
                range->table_offset =
                    table_base + strike->ranges_offset + FT_GET_ULONG();
                count2--;
                range++;
            }
...
tt_face_load_strike_metrics(TT_Face face, FT_ULong strike_index,
    ...
    /* XXX: Is this correct? */
    metrics->max_advance =
        (strike->hori.min_origin_SB + strike->hori.max_width +
...
/*                                                                       */
static FT_Error find_sbit_range(FT_UInt glyph_index, TT_SBit_Strike strike,
    ...
    for (; range < range_limit; range++) {
        if (glyph_index >= (FT_UInt)range->first_glyph &&
            ...
            switch (range->index_format) {
                ...
                case 2:
                    *aglyph_offset =
                        range->image_offset + range->image_size * delta;
                    break;
...
/*                                                                       */
FT_LOCAL(FT_Error)
tt_find_sbit_image(TT_Face face, FT_UInt glyph_index, FT_ULong strike_index,
                   TT_SBit_Range* arange, TT_SBit_Strike* astrike,
                   FT_ULong* aglyph_offset) {
    FT_Error error;
    TT_SBit_Strike strike;
...
/*                                                                       */
FT_LOCAL(FT_Error)
tt_load_sbit_metrics(FT_Stream stream, TT_SBit_Range range,
...
#define FT_STRUCTURE TT_SBit_SmallMetricsRec
        ...
        case 9:
            ...
            if (FT_STREAM_READ_FIELDS(sbit_metrics_fields, metrics)) goto Exit;
            break;
...
static void crop_bitmap(FT_Bitmap* map, TT_SBit_Metrics metrics) {
    ...
    Found_Top:
        ...
        if (count >= rows) goto Empty_Bitmap;
    ...
    do {
        ...
        for (; line < limit; line += line_len)
            if (line[0] & 0x80) goto Found_Left;
...
Found_Left:
    ...
    do {
        ...
        for (; line < limit; line += line_len)
            if (line[0] & mask) goto Found_Right;
...
static FT_Error Load_SBit_Single(FT_Bitmap* map, FT_Int x_offset,
                                 FT_Int y_offset, FT_Int pix_bits,
                                 FT_UShort image_format,
                                 TT_SBit_Metrics metrics, FT_Stream stream) {
    FT_Error error;
...

    {
        ...
        if (FT_FRAME_ENTER(glyph_size)) goto Exit;
...
static FT_Error Load_SBit_Image(TT_SBit_Strike strike, TT_SBit_Range range,
                                FT_ULong ebdt_pos, FT_ULong glyph_offset,
                                FT_GlyphSlot slot, FT_Int x_offset,
                                FT_Int y_offset, FT_Stream stream,
                                TT_SBit_Metrics metrics, FT_Int depth) {
    FT_Memory memory = stream->memory;
    ...
    if (depth == 0) {
        ...
        if (size == 0) goto Exit; /* exit successfully! */
    ...
    /* the array of elements.                                    */
    {
        ...
        if (FT_FRAME_ENTER(4L * num_components)) goto Fail_Memory;
        ...
        for (; count > 0; count--, comp++) {
            ...
            error = find_sbit_range(comp->glyph_code, strike, &elem_range,
                                    &elem_offset);
            if (error) goto Fail_Memory;

            /* now load the element, recursively */
            error = Load_SBit_Image(strike, elem_range, ebdt_pos, elem_offset,
                                    slot, x_offset + comp->x_offset,
                                    y_offset + comp->y_offset, stream,
                                    &elem_metrics, depth + 1);
            if (error) goto Fail_Memory;
        }
...
tt_face_load_sbit_image(TT_Face face, FT_ULong strike_index,
    ...
    error = tt_find_sbit_image(face, glyph_index, strike_index, &range, &strike,
                               &glyph_offset);
    if (error) goto Exit;
    ...
    if (load_flags & FT_LOAD_CROP_BITMAP) crop_bitmap(map, metrics);
