...
tt_face_load_eblc(TT_Face face, FT_Stream stream) {
    ...
    if (error) error = face->goto_table(face, TTAG_bloc, stream, &table_size);
    if (error) goto Exit;
    ...
    if (FT_FRAME_EXTRACT(table_size, face->sbit_table)) goto Exit;
    ...
    if (8 + 48UL * count > table_size) count = (FT_UInt)((p_limit - p) / 48);
...
static FT_Error tt_sbit_decoder_init(TT_SBitDecoder decoder, TT_Face face,
    ...
    if (error) error = face->goto_table(face, TTAG_bdat, stream, &ebdt_size);
    if (error) goto Exit;
...
static FT_Error tt_sbit_decoder_load_byte_aligned(TT_SBitDecoder decoder,
    ...
    if (x_pos < 0 || x_pos + width > bit_width || y_pos < 0 ||
        y_pos + height > bit_height) {
        error = FT_THROW(Invalid_File_Format);
        goto Exit;
    }
    ...
    if (x_pos == 0) /* the easy one */
    {
        for (h = height; h > 0; h--, line += pitch) {
            ...
            if (w > 0) write[0] = (FT_Byte)(write[0] | (*p++ & (0xFF00U >> w)));
        }
    } else /* x_pos > 0 */
    {
        for (h = height; h > 0; h--, line += pitch) {
            ...
            if (w > 0) wval = (FT_UInt)(wval | (*p++ & (0xFF00U >> w)));
...
static FT_Error tt_sbit_decoder_load_bit_aligned(TT_SBitDecoder decoder,
    ...
    for (h = height; h > 0; h--, line += pitch) {
        ...
        if (x_pos) {
            ...
            *write++ |=
                ((rval >> nbits) & 0xFF) & (~(0xFF << w) << (8 - w - x_pos));
            rval <<= 8;
...
static FT_Error tt_sbit_decoder_load_compound(TT_SBitDecoder decoder,
    ...
    if (p + 2 > limit) goto Fail;

    num_components = FT_NEXT_USHORT(p);
    if (p + 4 * num_components > limit) goto Fail;
    ...
    for (nn = 0; nn < num_components; nn++) {
        ...
        /* NB: a recursive call */
        error =
            tt_sbit_decoder_load_image(decoder, gindex, x_pos + dx, y_pos + dy);
        if (error) break;
    }
...
static FT_Error tt_sbit_decoder_load_image(TT_SBitDecoder decoder,
    ...
    for (; num_ranges > 0; num_ranges--) {
        ...
        if (glyph_index >= start && glyph_index <= end) goto FoundRange;
...
FoundRange:
    ...
    if (image_offset > (FT_ULong)(p_limit - p)) goto Failure;

    p += image_offset;
    if (p + 8 > p_limit) goto NoBitmap;
    ...
    switch (index_format) {
        ...
        case 4: /* sparse glyph array with (glyph,offset) pairs */
        {
            ...
            if (mm >= num_glyphs) goto NoBitmap;
        } break;
        ...
        case 5: /* constant metrics with sparse glyph codes */
        {
            ...
            if (num_glyphs > (FT_ULong)((p_limit - p) >> 1)) goto NoBitmap;

            for (mm = 0; mm < num_glyphs; mm++) {
                FT_UInt gindex = FT_NEXT_USHORT(p);

                if (gindex == glyph_index) break;
            }
