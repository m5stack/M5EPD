...
static FT_Error load_format_20(TT_Face face, FT_Stream stream,
    ...
    if (FT_READ_USHORT(num_glyphs)) goto Exit;
    ...
    /* load the indices */
    {
        ...
        for (n = 0; n < num_glyphs; n++) glyph_indices[n] = FT_GET_USHORT();
    ...
    /* compute number of names stored in table */
    {
        ...
        for (n = 0; n < num_glyphs; n++) {
            ...
            if (idx >= 258) {
                idx -= 257;
                if (idx > num_names) num_names = (FT_UShort)idx;
            }
        }
    }
    ...
    /* now load the name strings */
    {
        ...
        if (FT_NEW_ARRAY(name_strings, num_names)) goto Fail;
        ...
        for (n = 0; n < num_names; n++) {
            ...
            if (FT_STREAM_POS() >= post_limit)
                ...
                if (FT_READ_BYTE(len)) goto Fail1;
            }
...
Fail1 : {
    ...
    for (n = 0; n < num_names; n++) FT_FREE(name_strings[n]);
}
...
static FT_Error load_post_names(TT_Face face) {
    ...
    if (FT_STREAM_SKIP(32)) goto Exit;
...
tt_face_get_ps_name(TT_Face face, FT_UInt idx, FT_String** PSname) {
    ...
    if (!psnames) return FT_THROW(Unimplemented_Feature);
#endif
