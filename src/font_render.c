...
static esp_err_t font_cache_init(font_render_t *render) {
...
#endif
    render->max_pixel_width =
        (render->pixel_size * (render->font_face->ft_face->bbox.xMax -
                               render->font_face->ft_face->bbox.xMin)) /
            render->font_face->ft_face->units_per_EM +
        1;
    render->max_pixel_height =
        (render->pixel_size * (render->font_face->ft_face->bbox.yMax -
                               render->font_face->ft_face->bbox.yMin)) /
            render->font_face->ft_face->units_per_EM +
        1;
    // render->origin = (render->pixel_size *
    // (-render->font_face->ft_face->bbox.yMin)) /
    // render->font_face->ft_face->units_per_EM; render->bytes_per_glyph =
    // ((uint32_t)render->max_pixel_width * (uint32_t)render->max_pixel_height)
    // >> 1; render->bytes_per_glyph = ((uint32_t)(render->bytes_per_glyph + 1))
    // & 0xFFFFFFFE;
    render->bytes_per_glyph =
        ((render->pixel_size * render->pixel_size) >> 1) + 1;

    render->glyph_cache = (uint8_t *)heap_caps_malloc(
        render->bytes_per_glyph * render->cache_size, FONT_CACHE_ALLOC);
    render->glyph_cache_records = (glyph_cache_record_t *)heap_caps_malloc(
        sizeof(glyph_cache_record_t) * render->cache_size, FONT_CACHE_ALLOC);
    ...
    memset(render->glyph_cache_records, 0,
           sizeof(glyph_cache_record_t) * render->cache_size);
...
esp_err_t font_face_init(font_face_t *face, const font_data_t *data,
                         font_data_size_t size) {
    FT_Error err;
...
esp_err_t font_render_init(font_render_t *render, font_face_t *face,
                           font_size_t pixel_size, uint16_t cache_size) {
...
esp_err_t font_load_glyph_metrics(font_render_t *render, uint32_t utf_code) {
    if (font_face_set_pixel_size(render->font_face, render->pixel_size) !=
        ESP_OK) {
        return ESP_FAIL;
    }

    FT_UInt glyph_index =
        FT_Get_Char_Index(render->font_face->ft_face, utf_code);
    if (glyph_index == 0) {
        return ESP_FAIL;
    }
    ...
    FT_Error err;
    err =
        FT_Load_Glyph(render->font_face->ft_face, glyph_index, FT_LOAD_DEFAULT);
    if (err) {
        return ESP_FAIL;
    }
...
esp_err_t font_render_glyph(font_render_t *render, uint32_t utf_code) {
    ...
    if (found_cache == -1) {
        ...
        FT_Error err;
        err = FT_Load_Glyph(render->font_face->ft_face, glyph_index,
                            FT_LOAD_DEFAULT);
        if (err) {
            return ESP_FAIL;
        }

        err = FT_Render_Glyph(render->font_face->ft_face->glyph,
        ...
        render->glyph_cache_records[found_cache].priority =
            render->current_priority;
        render->glyph_cache_records[found_cache].utf_code = utf_code;
        render->glyph_cache_records[found_cache].metrics =
            render->font_face->ft_face->glyph->metrics;
        render->glyph_cache_records[found_cache].bitmap_width =
            render->font_face->ft_face->glyph->bitmap.width;
        render->glyph_cache_records[found_cache].bitmap_height =
            render->font_face->ft_face->glyph->bitmap.rows;
        render->glyph_cache_records[found_cache].bitmap_left =
            render->font_face->ft_face->glyph->bitmap_left;
        render->glyph_cache_records[found_cache].bitmap_top =
            render->font_face->ft_face->glyph->bitmap_top;
        render->glyph_cache_records[found_cache].advance =
            render->font_face->ft_face->glyph->advance.x >> 6;

        render->bitmap =
            render->glyph_cache + (render->bytes_per_glyph * found_cache);
        memset(render->bitmap, 0, render->bytes_per_glyph);
        size_t pos = 0;
        for (size_t y = 0; y < render->font_face->ft_face->glyph->bitmap.rows;
             ++y) {
            for (size_t x = 0;
                 x < render->font_face->ft_face->glyph->bitmap.width; ++x) {
                uint8_t color = render->font_face->ft_face->glyph->bitmap
                                    .buffer[y * render->font_face->ft_face
                                                    ->glyph->bitmap.pitch +
                                            x];
                color >>= 4;
                if (pos & 0x1) {
