    ...
    if [[ $SKETCHES_EXCLUDE == *"$SKETCH_BASENAME"* ]]; then
      ...
      if [[ $? -ne 0 ]]; then
        ...
        if [[ -z $GCC_EXTRA_FLAGS ]]; then
          ...
          echo "arduino-cli compile -e --fqbn ${ARDUINO_BOARD_FQBN%|*} $BUILD_PATH_PARAMETER $EXTRA_ARDUINO_CLI_ARGS $SKETCH_PATH"
        else
          ...
          echo "arduino-cli compile -e --fqbn ${ARDUINO_BOARD_FQBN%|*} $BUILD_PATH_PARAMETER --build-property compiler.cpp.extra_flags=\"${GCC_EXTRA_FLAGS}\" --build-property compiler.c.extra_flags=\"${GCC_EXTRA_FLAGS}\" --build-property compiler.S.extra_flags=\"${GCC_EXTRA_FLAGS}\" $EXTRA_ARDUINO_CLI_ARGS $SKETCH_PATH"
        fi
